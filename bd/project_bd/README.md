...
Для удобного получения сводных данных без необходимости писать сложные `JOIN` вручную, предусмотрено представление `v_citizen_details`. Оно возвращает полную, удобочитаемую карточку гражданина с названиями городов и стран вместо их ID.

## Тестирование системы
Для проверки корректности работы триггеров, процедур и представлений был разработан набор тестов (`sql/4_tests.sql`). 
* **Отчет о тестировании:** Результаты выполнения тестов зафиксированы в файле [docs/test_results.txt](docs/test_results.txt). В нем подтверждена работа представления, успешная запись миграции через процедуру и корректный пересчет населения через триггер.